...
static void ime_set_status(BOOL fOpen)
{
...
static BOOL ime_get_status(void)
{
...
DEFUN ("ime-save-and-set-status", Fime_save_and_set_status, Sime_save_and_set_status,
       1, 1, "", doc: /**/)
     (openp)
     Lisp_Object openp;
{
  ...
  /* MessageBox(h, "foo", "bar", MB_OK); */

  ime_status = ime_get_status();
  ime_set_status(XINT(openp));
  /* ime_set_status(FALSE); */

  return Qnil;
}


DEFUN ("ime-restore-status", Fime_restore_status, Sime_restore_status,
       0, 0, "", doc: /**/)
     ()
{
...
only be necessary if the default setting causes problems.  */);
  ...
  defsubr (&Sime_save_and_set_status);
  defsubr (&Sime_restore_status);
